...
from lreid.datasets import (IncrementalSamples4subcuhksysu, IncrementalSamples4market,
                               ...
                               IncrementalSamples4cuhk01, 
                            #    IncrementalSamples4cuhk02,
...
class IncrementalReIDLoaders:
    ...
    def _load(self):
        ...
        '''init train dataset'''
        train_samples = self._get_train_samples(self.config.train_dataset)# record all person images in a list, the ID is accumulated dataset by dataset, total length:71448
        ...
        for step_index, pid_per_step in self.global_pids_per_step_dict.items():
            if self.config.num_identities_per_domain is -1:
                ...
                one_step_pid_list = sorted(list(pid_per_step))[0:self.config.num_identities_per_domain] # keep first 500 ids for each domain
            temp_dict[step_index] = one_step_pid_list # record selected ids for each domain
            total_pid_list.extend(one_step_pid_list) # record all ids for in the training stage
        num_of_real_train = 0 # count all the training images
        ...
        for cid_per_step in self.global_cids_per_step_dict.values(): # record all the camreas in actual training 
        ...
        '''xkl add'''
        max_id=max(total_pid_list) # obtain the max global id 
        self.mappedID=torch.zeros(max_id+1,dtype=torch.int32)
        S_id=0
        for k,v in self.global_pids_per_step_dict.items():
            for new_local, global_id in enumerate(v):
                self.mappedID[global_id]=new_local+S_id   
            S_id+=len(v)     
