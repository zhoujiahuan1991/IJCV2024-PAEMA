...
class ResNet(nn.Module):
    def __init__(self, last_stride, bn_norm, with_ibn, with_se, block, num_class, layers):
        ...
        self.classifier = nn.Linear(512*block.expansion, num_class, bias=False)
        nn.init.normal_(self.classifier.weight, std=0.001)

        self.task_specific_batch_norm = nn.ModuleList(nn.BatchNorm2d(512*block.expansion) for _ in range(4))
...
def build_resnet_backbone(num_class, depth, pretrain=True):
    ...
    if pretrain:
        cached_file = '/home/liqiwei/.cache/torch/checkpoints/resnet50-19c8e357.pth'
